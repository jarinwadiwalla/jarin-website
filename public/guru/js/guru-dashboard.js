/* ==========================================================================
   ...
   Stats, newsletter, subscribers, todos, focus banner, reading goal, sobriety
   ========================================================================== */
...
// ==================== FOCUS BANNER ====================
// Consolidated settings loader — fetches /api/settings once for focus banner, reading goal, and sobriety milestones
async function loadDashboardSettings() {
    try {
        ...
        // Sobriety milestones
        sobrietyMilestones = data.sobrietyMilestones || [];
        renderSobrietyMilestones();
        updateSobrietyCardSub();
    } catch (e) {}
}
...
// ==================== SOBRIETY MILESTONES ====================
let sobrietyMilestones = [];
let editingMilestoneIdx = -1;
let selectedMilestoneColor = '#8b5cf6';

function toggleSobrietyPanel() {
    const panel = document.getElementById('sobrietyPanel');
    panel.style.display = panel.style.display === 'none' ? 'block' : 'none';
}

// Entry point for the dashboard jump-to row — always opens (never toggles closed)
function openSobrietyPanel() {
    document.getElementById('sobrietyPanel').style.display = 'block';
}

// Welcome line date — static per page load, e.g. "Thursday · Jun 12"
(function () {
...
async function loadSobrietyMilestones() {
    try {
        const data = await apiFetch('/api/settings');
        sobrietyMilestones = data.sobrietyMilestones || [];
        renderSobrietyMilestones();
...
async function saveSobrietyToSettings() {
    try {
        await apiFetch('/api/settings', {
            method: 'POST',
            body: JSON.stringify({ sobrietyMilestones }),
        });
    } catch (e) {
...
function calcSobriety(startDate) {
    ...
    return { totalDays, totalHours, years, months, days, hours, minutes };
}

function formatSobrietyDetail(s) {
    const parts = [];
    if (s.years > 0) parts.push(s.years + ' year' + (s.years !== 1 ? 's' : ''));
    if (s.months > 0) parts.push(s.months + ' month' + (s.months !== 1 ? 's' : ''));
    if (s.days > 0 || parts.length === 0) parts.push(s.days + ' day' + (s.days !== 1 ? 's' : ''));
    parts.push(s.hours + 'h ' + s.minutes + 'm');
    return parts.join(', ');
}

function renderSobrietyMilestones() {
    const container = document.getElementById('sobrietyMilestones');
    if (!sobrietyMilestones.length) {
        container.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:32px;color:var(--gray-400);">No milestones yet. Click "+ Add Milestone" to start tracking.</div>';
        return;
    }

    container.innerHTML = sobrietyMilestones.map((m, i) => {
        const s = calcSobriety(m.date);
        const textColor = isLightColor(m.color) ? '#1f2937' : '#ffffff';
        const resets = m.resets || [];
        const resetCount = resets.length;
        return `<div class="ms-badge" draggable="true" data-idx="${i}"
            style="background:${m.color};color:${textColor};"
            ondragstart="msDragStart(event,${i})" ondragover="event.preventDefault()" ondrop="msDrop(event,${i})">
            <div class="ms-actions">
                <button onclick="event.stopPropagation();openResetMilestone(${i})" title="Reset">&#8635;</button>
                <button onclick="event.stopPropagation();editMilestone(${i})" title="Edit">&#9998;</button>
                <button onclick="event.stopPropagation();deleteMilestone(${i})" title="Delete">&#10005;</button>
            </div>
            <div class="ms-days">${s.totalDays}</div>
            <div class="ms-label">days, ${s.hours}h ${s.minutes}m</div>
            <div class="ms-name">${esc(m.name)}</div>
            <div class="ms-detail">${formatSobrietyDetail(s)}</div>
            <div class="ms-detail" style="margin-top:2px;">since ${new Date(m.date.includes('T') ? m.date : m.date + 'T00:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}${m.date.includes('T') ? ' at ' + m.date.slice(11, 16) : ''}</div>
            ${resetCount > 0 ? `<div class="ms-resets" onclick="event.stopPropagation();openResetHistory(${i})">${resetCount} reset${resetCount !== 1 ? 's' : ''}</div>` : ''}
        </div>`;
    }).join('');
}

function updateSobrietyCardSub() {
    const el = document.getElementById('sobrietyCardSub');
    if (!el) return;
    if (sobrietyMilestones.length) {
        el.textContent = sobrietyMilestones.length + ' milestone' + (sobrietyMilestones.length !== 1 ? 's' : '') + ' tracked';
    } else {
        el.textContent = 'Track your milestones';
    }
}
...
function openAddMilestone() {
    ...
    document.getElementById('milestoneDateInput').value = new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 16);
    selectMilestoneColor('#8b5cf6');
    document.getElementById('milestoneModal').style.display = 'flex';
    document.getElementById('milestoneNameInput').focus();
}

function editMilestone(idx) {
    editingMilestoneIdx = idx;
    const m = sobrietyMilestones[idx];
    document.getElementById('milestoneModalTitle').textContent = 'Edit Milestone';
    document.getElementById('milestoneNameInput').value = m.name;
    // Handle legacy date-only values for datetime-local input
    document.getElementById('milestoneDateInput').value = m.date.includes('T') ? m.date : m.date + 'T00:00';
    selectMilestoneColor(m.color);
    document.getElementById('milestoneModal').style.display = 'flex';
}
...
function selectMilestoneColor(color) {
    selectedMilestoneColor = color;
...
async function saveMilestone() {
    ...
    const milestone = { name, date, color: selectedMilestoneColor };

    if (editingMilestoneIdx >= 0) {
        // Preserve resets history when editing
        milestone.resets = sobrietyMilestones[editingMilestoneIdx].resets || [];
        sobrietyMilestones[editingMilestoneIdx] = milestone;
    } else {
        milestone.resets = [];
        sobrietyMilestones.push(milestone);
    }

    renderSobrietyMilestones();
    updateSobrietyCardSub();
    closeMilestoneModal();
    await saveSobrietyToSettings();
    showToast(editingMilestoneIdx >= 0 ? 'Milestone updated!' : 'Milestone added!', 'success');
}

async function deleteMilestone(idx) {
    if (!confirm('Delete "' + sobrietyMilestones[idx].name + '"?')) return;
    sobrietyMilestones.splice(idx, 1);
    renderSobrietyMilestones();
    updateSobrietyCardSub();
    await saveSobrietyToSettings();
...
async function msDrop(e, dropIdx) {
    e.preventDefault();
    if (msDragIdx < 0 || msDragIdx === dropIdx) return;
    const item = sobrietyMilestones.splice(msDragIdx, 1)[0];
    sobrietyMilestones.splice(dropIdx, 0, item);
    msDragIdx = -1;
    renderSobrietyMilestones();
    await saveSobrietyToSettings();
}
...
// ==================== SOBRIETY RESET ====================
let resettingMilestoneIdx = -1;

function openResetMilestone(idx) {
    resettingMilestoneIdx = idx;
    const m = sobrietyMilestones[idx];
    const s = calcSobriety(m.date);
    const sinceDate = new Date(m.date.includes('T') ? m.date : m.date + 'T00:00:00')
        .toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
    document.getElementById('resetMilestoneTitle').textContent = 'Reset: ' + m.name;
    document.getElementById('resetMilestoneSummary').innerHTML =
        `You have been clean for <strong>${s.totalDays} days</strong> (${formatSobrietyDetail(s)}) since ${sinceDate}. This will record your streak and start a new counter.`;
    document.getElementById('resetMilestoneDateInput').value =
        new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 16);
    document.getElementById('resetMilestoneNote').value = '';
    document.getElementById('resetMilestoneModal').style.display = 'flex';
}

function closeResetModal() {
    document.getElementById('resetMilestoneModal').style.display = 'none';
}

async function confirmResetMilestone() {
    const idx = resettingMilestoneIdx;
    if (idx < 0) return;
    const m = sobrietyMilestones[idx];
    const newDate = document.getElementById('resetMilestoneDateInput').value;
    const note = document.getElementById('resetMilestoneNote').value.trim();
    if (!newDate) { showToast('New start date is required', 'error'); return; }
    ...
    const resetEntry = {
        ...
        newDate: newDate,
        daysClean: s.totalDays,
        note: note,
    };

    if (!m.resets) m.resets = [];
    m.resets.push(resetEntry);
    m.date = newDate;
    ...
    closeResetModal();
    await saveSobrietyToSettings();
    showToast(m.name + ' counter reset. Previous streak: ' + s.totalDays + ' days.', 'info');
}

function openResetHistory(idx) {
    const m = sobrietyMilestones[idx];
    const resets = m.resets || [];
    document.getElementById('resetHistoryTitle').textContent = m.name + ' — Reset History';

    // Stats
    const totalResets = resets.length;
    const streaks = resets.map(r => r.daysClean);
    const currentStreak = calcSobriety(m.date).totalDays;
    const allStreaks = [...streaks, currentStreak];
    const avgStreak = allStreaks.length ? Math.round(allStreaks.reduce((a, b) => a + b, 0) / allStreaks.length) : 0;
    const longestStreak = Math.max(...allStreaks, 0);

    document.getElementById('resetHistoryStats').innerHTML = `
        <div style="display:flex;gap:12px;flex-wrap:wrap;">
            <div style="flex:1;min-width:80px;text-align:center;padding:12px;background:var(--gray-50);border-radius:8px;">
                <div style="font-size:24px;font-weight:800;color:var(--gray-800);">${totalResets}</div>
                <div style="font-size:11px;color:var(--gray-500);">Total Resets</div>
            </div>
            <div style="flex:1;min-width:80px;text-align:center;padding:12px;background:var(--gray-50);border-radius:8px;">
                <div style="font-size:24px;font-weight:800;color:var(--gray-800);">${currentStreak}</div>
                <div style="font-size:11px;color:var(--gray-500);">Current Streak</div>
            </div>
            <div style="flex:1;min-width:80px;text-align:center;padding:12px;background:var(--gray-50);border-radius:8px;">
                <div style="font-size:24px;font-weight:800;color:var(--gray-800);">${longestStreak}</div>
                <div style="font-size:11px;color:var(--gray-500);">Longest Streak</div>
            </div>
            <div style="flex:1;min-width:80px;text-align:center;padding:12px;background:var(--gray-50);border-radius:8px;">
                <div style="font-size:24px;font-weight:800;color:var(--gray-800);">${avgStreak}</div>
                <div style="font-size:11px;color:var(--gray-500);">Avg Streak</div>
            </div>
        </div>`;

    // List of resets (newest first)
    if (resets.length === 0) {
        document.getElementById('resetHistoryList').innerHTML =
            '<div style="text-align:center;padding:20px;color:var(--gray-400);">No resets yet.</div>';
    } else {
        document.getElementById('resetHistoryList').innerHTML = resets.slice().reverse().map((r, i) => {
            const resetDate = new Date(r.resetAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
            return `<div class="ms-reset-entry">
                <div class="ms-reset-date">Reset #${resets.length - i} — ${resetDate}</div>
                <div class="ms-reset-streak">${r.daysClean} day${r.daysClean !== 1 ? 's' : ''} clean before reset</div>
                ${r.note ? `<div class="ms-reset-note">"${esc(r.note)}"</div>` : ''}
            </div>`;
        }).join('');
    }

    document.getElementById('resetHistoryModal').style.display = 'flex';
}
